  ...
  for (i in seq_along(bodyArgs)) {
    ...
    if (all(bodyArgs[[i]] != "") || inherits(x = bodyArgs[[i]], what = c("data.frame", "list"))) {
      if (inherits(x = bodyArgs[[i]], what = "character") &&
          length(bodyArgs[[i]]) == 1 &&
          grepl(pattern = "(studyDbId)|(Addresses)|(Classes)|(Codes)|(Genus)|(genus)|(Ids)|(IDs)|(Links)|(Names)|(Numbers)|(PUIs)|(seasons)|(Sources)|(Species)|(synonyms)|(Terms)|(tions)|(Xrefs)|(ypes)|(markerDbId)|(markerProfileDbId)|(matrixDbId)|(objectives)",
                x = names(bodyArgs[i]))) {
